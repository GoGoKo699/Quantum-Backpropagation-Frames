...
```math
t_{b,j}=U_{b,<j}^{\mathsf T}(-iA_j)U_{b,<j}|00\rangle
...
B_{\rm sparse}=4P-8
  =\inf_{\text{universally unbiased one-copy POVMs}}
       \sup_q\operatorname{tr}\operatorname{Cov}_q(Z).
```
...
```math
\theta_b=\operatorname{atan2}(\sqrt{R_{b+1}},\sqrt{w_b}).
```
...
```math
\alpha_b=\operatorname{atan2}(\sqrt{d_{b,2}+d_{b,3}},\sqrt{d_{b,1}}).
```

Then rotate the right bit conditioned on left=1 and reference=0 by

```math
\gamma_b=-\operatorname{atan2}(\sqrt{d_{b,2}},\sqrt{d_{b,3}}).
```
