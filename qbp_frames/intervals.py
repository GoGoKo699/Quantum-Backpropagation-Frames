"""Interval-row reference compiler. Real full-angle Ry, big-endian sites.
...
import math
import numpy as np
from .parity import _real, _integer


def real(value, ndim):
    return _real(value, 'array', ndim)


def integer(value, low=0):
    return _integer(value, 'integer', low)
...
def fwht(v):
    ...
    try:
        ...
        raise ValueError('Walsh transform exceeds float64 range') from exc
    return v
...
class IntervalReference:
    ...
    def from_tables(cls,n,intervals,tables):
        try:
            with np.errstate(over='raise', invalid='raise', divide='raise'):
                return cls._from_tables(n, intervals, tables)
        except (TypeError, OverflowError, FloatingPointError) as exc:
            raise ValueError('invalid interval tables or unsupported numerical range') from exc

    @classmethod
    def _from_tables(cls,n,intervals,tables):
    ...
    def fill_gates(self,a):
        ...
        a=integer(a)
        if a>=self.n: raise ValueError('first site outside register')
    ...
    def preparation_gates(self,descending=True):
        if not isinstance(descending, bool): raise ValueError('descending must be boolean')
    ...
    def add_record(self,accumulator,b,bits):
        ...
        if (not isinstance(accumulator,np.ndarray) or accumulator.shape!=(self.P,) or
                accumulator.dtype != np.dtype('float64') or not accumulator.flags.writeable):
            raise ValueError('writable float64 P-vector accumulator required')
        ids,v=self.score(b,bits)
        if ids:
            index=np.asarray(ids,dtype=int)
            # Scan only touched coordinates; a full P-vector validation here
            # would destroy the sparse streaming cost. finish checks all P.
            if not np.isfinite(accumulator[index]).all():
                raise ValueError('nonfinite touched accumulator coordinates')
            try:
                with np.errstate(over='raise', invalid='raise'):
                    updated=accumulator[index]+v
            except FloatingPointError as exc:
                raise ValueError('accumulator exceeds float64 range') from exc
            accumulator[index]=updated
        return len(ids)

    def finish(self,accumulator,shots):
        """Divide accumulated scores by ALL records, including zero-score records."""
        accumulator=real(accumulator,1); shots=integer(shots,1)
        if accumulator.shape!=(self.P,) or shots>2**53:
            raise ValueError('P-vector and shots <= 2**53 required')
